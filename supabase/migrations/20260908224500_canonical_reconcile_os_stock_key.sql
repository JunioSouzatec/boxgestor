-- Canonicaliza números da chave de idempotência de estoque.
-- Ex.: 0, 0.0 e 0.00 resultam em "0"; 2, 2.0 e 2.00 resultam em "2".
...
BEGIN
  ...
  v_office_atual := public.current_office_id();
  v_is_admin := COALESCE(public.is_system_admin(), false);

  IF NOT v_is_admin THEN
    IF v_office_atual IS NULL OR p_office_id IS DISTINCT FROM v_office_atual THEN
  ...
  LOOP
    ...
    -- Forma canônica sem locale e sem zeros/ponto finais: 0.000 -> 0; 2.500 -> 2.5.
    v_chave := format(
      'os-delta:%s:%s:%s->%s',
      p_os_local_id,
      v_row.item_local_id,
      trim(trailing '.' from trim(trailing '0' from round(v_applied, 3)::text)),
      trim(trailing '.' from trim(trailing '0' from round(v_row.desired_qty, 3)::text))
    );
    ...
    SELECT quantity INTO v_qty_final
    FROM public.inventory_items
    WHERE id = v_item.id;
...
REVOKE ALL ON FUNCTION public.reconcile_os_stock(UUID, TEXT, INTEGER, JSONB, UUID, TEXT)
  FROM PUBLIC;
REVOKE ALL ON FUNCTION public.reconcile_os_stock(UUID, TEXT, INTEGER, JSONB, UUID, TEXT)
  FROM anon;
GRANT EXECUTE ON FUNCTION public.reconcile_os_stock(UUID, TEXT, INTEGER, JSONB, UUID, TEXT)
  TO authenticated;
GRANT EXECUTE ON FUNCTION public.reconcile_os_stock(UUID, TEXT, INTEGER, JSONB, UUID, TEXT)
  TO service_role;
