-- Harden reconcile_os_stock: evita bypass quando current_office_id() é NULL
-- e remove EXECUTE da role anon (só authenticated deve chamar).
...
BEGIN
  ...
  -- Evita 3-valued logic: NOT (NULL OR false) não entra no IF e liberava anon
...
REVOKE ALL ON FUNCTION public.reconcile_os_stock(UUID, TEXT, INTEGER, JSONB, UUID, TEXT) FROM anon;
GRANT EXECUTE ON FUNCTION public.reconcile_os_stock(UUID, TEXT, INTEGER, JSONB, UUID, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.reconcile_os_stock(UUID, TEXT, INTEGER, JSONB, UUID, TEXT) TO service_role;
